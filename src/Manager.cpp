...
void Manager::OnDataLoaded()
{
	...
	const auto gameMaxDistance = RE::INISettingCollection::GetSingleton()->GetSetting("fMaxSubtitleDistance:Interface")->GetFloat();
	...
	static RE::Setting* speaker_name_color = RE::INIPrefSettingCollection::GetSingleton()->GetSetting("iSubtitleSpeakerNameColor:Interface");
	
	speakerColorU32 = speaker_name_color ? speaker_name_color->GetSInt() : 8947848; // the default entry is 8947848
...
{
	return RE::INIPrefSettingCollection::GetSingleton()->GetSetting("bGeneralSubtitles:Interface")->GetBool() && settings.current.showGeneralSubtitles;
}
...
{
	return RE::INIPrefSettingCollection::GetSingleton()->GetSetting("bDialogueSubtitles:Interface")->GetBool() && settings.current.showDialogueSubtitles && !ModAPIHandler::GetSingleton()->ACCInstalled();
}
...
void Manager::CalculateVisibility(RE::SubtitleInfoEx& a_subInfo)
{
	...
	switch (RayCaster(actor).GetResult(true)) {
